...
def evaluate():
  ...
  if periodic:
    ...
    active_atoms = 3
    charge = 1
    atom_str_xyz = "7/n" + " " + atom_str

    # basis and xc functional are defined on lines 64 and 65
    nbed_config = {
        "geometry" : atom_str_xyz,
    ...
    nbed_driver = NbedDriver(**nbed_config)

    if isinstance(nbed_driver.embedded_scf.mo_occ[0], Number):
        AOs = nbed_driver.embedded_scf.mo_coeff.shape[0]
        MOs = nbed_driver.embedded_scf.mo_coeff.shape[1]
        mo_occ = nbed_driver.embedded_scf.mo_occ
        act_occ = np.count_nonzero(mo_occ)
    elif isinstance(nbed_driver.embedded_scf.mo_occ[0], np.ndarray):
        AOs = nbed_driver.embedded_scf.mo_coeff[0].shape[0]
        MOs = nbed_driver.embedded_scf.mo_coeff[0].shape[1]
        assert AOs == nbed_driver.embedded_scf.mo_coeff[1].shape[0]
        assert MOs == nbed_driver.embedded_scf.mo_coeff[1].shape[1]
        mo_occ = np.vstack((nbed_driver.embedded_scf.mo_occ[0], nbed_driver.embedded_scf.mo_occ[1]))
        act_occ = np.count_nonzero(mo_occ)/2
  ...
  if mode == QMMM:
    if periodic:
        mf = RKS_pbc(cell,xc=xcstr)
    else:
        mf = RKS_nonpbc(mol,xc=xcstr)

    mf = qmmm.mm_charge(mf,mm_coords,mm_charges,mm_radii)

    ham_builder = HamiltonianBuilder(
        scf_method = nbed_driver.embedded_scf,
        constant_e_shift = nbed_driver.classical_energy,
        auto_freeze_core = True
    )

    ham = ham_builder.reduce()

    ham_dict = ham.to_dictionary
    for pauli in ham_dict:
        ham_dict[pauli] = (ham_dict[pauli].real, ham_dict[pauli].imag)

    all_hams[''.join([str(step_count), '_ham'])] = ham_dict
    ...
    # 0-based counter for the MM step
    step_count += 1
...
if __name__== "__main__":
  ...
  # dump all coeffs to compressed pickle file:
  with open('output.pkl', 'wb') as outfile:
    pickle.dump(all_hams, outfile)
